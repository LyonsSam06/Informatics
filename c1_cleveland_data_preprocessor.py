...
# Handles missing data in the cleveland dataset.
# if missing percent > 40%, drops missing data
# else, fills with the mean value (mean imputation)
# Takes a DataFrame at input and returns one at output.
def handle_missing_data(data: pd.DataFrame) -> pd.DataFrame:
    percent_missing = data.isnull().sum().sum() * 100 / len(data)
...
# handles outlier data in the cleveland dataset.
# removes data below first quartile and above third quartile
# Takes a DataFrame at input and returns one at output.
