...
class Evaluator:
    ...
    # NEW individual fairness (FTA) evaluation function using gower distance. The function takes as input: the discretised features
    # of the dataset and the predicted classes from the model. It first computes the gower distance between all pairs of samples in the dataset and prediction similarity.
    def fairness_through_awareness(self,features, predictions):
        
        # Compute gower distance between all pairs of samples in the dataset
        distances = gower.gower_matrix(features)

        # Convert predictions to numpy array for indexing
        predictions = predictions.to_numpy()

        # Variables to count the number of violations and valid pairs
        violations = 0
        total = 0

        n = len(features)

        # Nested loop to compare each pair of samples
        for i in range(n):
            for j in range(i + 1, n):

                # Calculate the distance between the predicted classes and the gower distance between the samples
                dX = distances[i][j]
                dS = 0 if predictions[i] == predictions[j] else 1

                # Computes Gower distance between feature vectors and counts a violation when prediction disagreement exceeds feature-space distance
                if dS > dX:
                    violations += 1

                total += 1

        # Return the final result, rounded to 6 decimal places in line with the precision of other results.
        return "Individual fairness through awareness score: " + str(round(1 - (violations / total), 6))